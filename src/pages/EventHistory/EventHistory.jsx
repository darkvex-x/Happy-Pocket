      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-3">
             <h1 className="font-heading text-xl md:text-2xl font-black text-[var(--text-primary)] uppercase tracking-tight">Event History</h1>
            {!isLoading && events.length > 0 && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[var(--primary)]/10 text-[var(--primary)] border border-[var(--primary)]/20 uppercase tracking-wider">
...
{isLoading ? (
         ...
         <div className="overflow-y-auto pr-1" style={{ maxHeight: 'calc(100vh - 220px)' }}>
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-4">
             ...
             <Card
               key={event.id}
               className="border border-[var(--border)] bg-[var(--card)] shadow-md flex flex-col group hover:shadow-lg hover:border-[var(--primary)]/25 transition-all duration-300 relative overflow-hidden"
             >
               {/* Decorative Gradient Background */}
               <div className="absolute top-0 right-0 -mr-12 -mt-12 w-32 h-32 bg-gradient-to-br from-[var(--primary)]/5 to-transparent rounded-full pointer-events-none blur-2xl group-hover:from-[var(--primary)]/10 transition-colors"></div>

               <CardHeader className="pb-4">
                  <CardTitle className="font-heading text-lg font-bold text-[var(--text-primary)] line-clamp-1 pr-8">
                   {event.eventName}
                 </CardTitle>
                 <div className="text-xs font-semibold text-[var(--muted)] mt-1 line-clamp-1">
               ...
               <CardContent className="flex-1 space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                   <div className="bg-[var(--background-secondary)] p-4 rounded-xl flex items-center space-x-3 border border-[var(--border)]">
                     <div className="bg-[var(--success)]/10 text-[var(--success)] p-2 rounded-lg">
                       <IndianRupee size={16} />
                     </div>
                     <div>
                       <div className="text-[10px] uppercase font-bold text-[var(--muted)] tracking-wider">
                        ...
                        <div
                          className="font-bold text-[var(--text-primary)] leading-tight block truncate sm:max-w-24 md:max-w-none text-sm font-number"
                   ...
                   <div className="bg-[var(--background-secondary)] p-4 rounded-xl flex items-center space-x-3 border border-[var(--border)]">
                     ...
                     <div>
                       <div className="text-[10px] uppercase font-bold text-[var(--muted)] tracking-wider">
                        ...
                        <div className="font-bold text-[var(--text-primary)] leading-tight block truncate text-sm font-number">
                          {event.totalEntries}
                        </div>
                     </div>
                   </div>
               ...
               <CardFooter className="border-t border-[var(--border)] bg-[var(--card-secondary)] py-3 px-4 grid grid-cols-4 gap-2">
                <Button
                  variant="secondary"
                  size="sm"
                ...
                {permissions.includes(PERMISSIONS.DELETE_EVENT) && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="w-full text-xs h-8 px-2 text-[var(--danger)] hover:bg-[var(--danger)]/10 hover:text-red-300"
                    title="Delete Event"
